...
#define PAGING_PAGESZ  256      /* 256B or 8-bits PAGE NUMBER */
#define PAGING_PTE_PRESENT_MASK  (1U << 0)
#define PAGING_PTE_SWAPPED_MASK  (1U << 1)
#define PAGING_PTE_FPN_MASK  GENMASK(19, 10)
#define PAGING_PTE_FPN_LOBIT  10
#define PAGING_CPU_BUS_WIDTH  1024

#define BIT(nr)                 (1U << (nr))
#define GENMASK(h, l) \
  (((~0U) << (l)) & (~0U >> (32 - (h) - 1)))
#define NBITS(n) \
  (n == 0 ? 0 : ((n & 0xFFFF0000) ? (16 + NBITS(n >> 16)) : (NBITS(n))))
#define GETVAL(v, mask, offst) \
  ((v & mask) >> offst)
...
int main()
{
  uint32_t *pte5 = malloc(PAGING_CPU_BUS_WIDTH * sizeof(uint32_t));
  pte_set_fpn(pte5, 4);
  printf("%d\n", PAGING_FPN(*pte5));
